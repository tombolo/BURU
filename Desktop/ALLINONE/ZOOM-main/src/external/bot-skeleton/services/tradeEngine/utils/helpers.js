...
export const createDetails = contract => {
    const { sell_price: sellPrice, buy_price: buyPrice, currency, exit_tick, is_expired } = contract;
    // Use sell_price if available, otherwise use exit_tick for expired contracts
    const exitPrice = sellPrice !== undefined ? sellPrice : (is_expired && exit_tick !== undefined ? exit_tick : buyPrice);
